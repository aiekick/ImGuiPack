...
ImVec2 ax::NodeEditor::GetCanvasOffset() {
    return s_Editor->GetView().Origin;
}

float ax::NodeEditor::GetCanvasScale() {
    return s_Editor->GetView().Scale;
}

void ax::NodeEditor::SetCanvasView(const ImVec2& vOffset, const float vScale) {
    s_Editor->NavigateTo(vOffset, vScale);
}
