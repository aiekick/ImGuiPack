...
float CalcContrastRatio(const ImU32& backgroundColor, const ImU32& foreGroundColor) {
    ...
    const float contrastRatio = (sr * sa0 * ((backgroundColor >> IM_COL32_R_SHIFT) & 0xFF) +
...
bool PushStyleColorWithContrast1(const ImGuiCol& backGroundColor, const ImGuiCol& foreGroundColor,
                                 const ImU32& invertedColor, const float& maxContrastRatio) {
...
bool PushStyleColorWithContrast2(const ImGuiCol& backGroundColor, const ImGuiCol& foreGroundColor,
                                 const ImVec4& invertedColor, const float& maxContrastRatio) {
    const float contrastRatio = CalcContrastRatio(GetColorU32(backGroundColor), GetColorU32(foreGroundColor));
    if (contrastRatio < maxContrastRatio) {
        PushStyleColor(foreGroundColor, invertedColor);
...
bool PushStyleColorWithContrast3(const ImU32& backGroundColor, const ImGuiCol& foreGroundColor,
                                 const ImVec4& invertedColor, const float& maxContrastRatio) {
...
bool PushStyleColorWithContrast4(const ImU32& backGroundColor, const ImGuiCol& foreGroundColor,
                                 const ImU32& invertedColor, const float& maxContrastRatio) {
    const float contrastRatio = CalcContrastRatio(backGroundColor, GetColorU32(foreGroundColor));
    if (contrastRatio < maxContrastRatio) {
        PushStyleColor(foreGroundColor, invertedColor);
...
inline void PathInvertedRect(ImDrawList* vDrawList, const ImVec2& a, const ImVec2& b, ImU32 col, float rounding,
...
void AddInvertedRectFilled(ImDrawList* vDrawList, const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding,
                           ImDrawFlags rounding_corners) {
    if (!vDrawList) return;

    if ((col & IM_COL32_A_MASK) == 0) return;
    if (rounding > 0.0f) PathInvertedRect(vDrawList, p_min, p_max, col, rounding, rounding_corners);
}

// Render a rectangle shaped with optional rounding and borders
void RenderInnerShadowFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, ImU32 fill_col_darker, ImU32 bg_Color,
    ...
    if (border && border_size > 0.0f) {
        window->DrawList->AddRect(p_min + ImVec2(1, 1), p_max + ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow),
                                  rounding, ImDrawFlags_RoundCornersAll, border_size);
        window->DrawList->AddRect(p_min, p_max, GetColorU32(ImGuiCol_Border), rounding, ImDrawFlags_RoundCornersAll,
                                  border_size);
    }
}
...
bool ImageCheckButton(ImTextureID user_texture_id, bool* v, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1,
                      const ImVec2& vHostTextureSize, int frame_padding, float vRectThickNess, ImVec4 vRectColor) {
    ImGuiWindow* window = GetCurrentWindow();
    ...
    // Render
    const ImU32 col = GetColorU32(((held && hovered) || (v && *v)) ? ImGuiCol_ButtonActive
                                  : hovered                        ? ImGuiCol_ButtonHovered
                                                                   : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, style.FrameRounding));
    if (vRectThickNess > 0.0f) {
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(vRectColor), 0.0, ImDrawFlags_RoundCornersAll,
                                  vRectThickNess);
    }

    // resize with respect to glyph ratio
    float hostRatioX = 1.0f;
    if (vHostTextureSize.y > 0) hostRatioX = ImRatioX(vHostTextureSize);
    ...
    window->DrawList->AddImage(user_texture_id, center - glyphSize, center + glyphSize, uv0, uv1,
...
void FramedGroupSeparator() {
    ...
    if (ItemAdd(bb, 0)) {
        ...
        window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y - style.FramePadding.y * 0.5f),
...
void FramedGroupText(ImVec4* vTextColor, const char* vHelp, const char* vFmt, va_list vArgs) {
    ...
    if (w) {
        ...
        const float frame_height = ImMax(ImMin(window->DC.CurrLineSize.y, g.FontSize + style.FramePadding.y),
...
bool CheckBoxIcon(const char* vLabel, const char* vIconTrue, bool* v) {
    ...
    if (label_size.x > 0.0f) RenderText(label_pos, vLabel);
...
bool RadioFloatDefault(const char* vName, float* vVar, int vCount, float* vDefault, const char* vHelp,
                       ImFont* vLabelFont) {
    bool change = false;
    ...
    BeginGroup();
    {
        ...
        for (int i = 0; i < vCount; ++i) {
            if (i > 0) SameLine(0, GetStyle().ItemInnerSpacing.x);
            PushID(IncPUSHID());
            bool v = (vVar[i] > 0.5f);
            if (Checkbox("##radio", &v)) {
                radioSelectedId = i;
...
bool RadioButtonLabeled(float vWidth, const char* label, bool active, bool disabled) {
    ...
    if (style.FrameBorderSize > 0.0f) {
        window->DrawList->AddRect(total_bb.Min + ImVec2(1, 1), total_bb.Max, GetColorU32(ImGuiCol_BorderShadow),
                                  style.FrameRounding);
        window->DrawList->AddRect(total_bb.Min, total_bb.Max, GetColorU32(ImGuiCol_Border), style.FrameRounding);
    }

    if (label_size.x > 0.0f) {
        const bool pushed = PushStyleColorWithContrast1(colUnderText, ImGuiCol_Text, CustomStyle::puContrastedTextColor,
                                                        CustomStyle::puContrastRatio);
...
bool RadioButtonLabeled(float vWidth, const char* label, const char* help, bool active, bool disabled,
                        ImFont* vLabelFont) {
...
bool RadioButtonLabeled(ImVec2 vSize, const char* label, const char* help, bool active, bool disabled,
                        ImFont* vLabelFont) {
    if (vLabelFont) PushFont(vLabelFont);
...
bool CollapsingHeader_SmallHeight(const char* vName, float vHeightRatio, float vWidth, bool vDefaulExpanded,
                                  bool* vIsOpen) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    const ImVec2 pos   = window->DC.CursorPos;
    const ImVec2 nsize = CalcItemSize(ImVec2(vWidth, label_size.y + padding.y * 2.0f), label_size.x + padding.x * 2.0f,
                                      label_size.y + padding.y * 2.0f);
    ...
    const ImU32 col = GetColorU32((held && hovered) ? _ScrollbarGrabActive
                                  : hovered         ? _ScrollbarGrabHovered
                                                    : _ScrollbarGrab);
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    RenderTextClipped(bb.Min, bb.Max - padding, vName, nullptr, &label_size, style.ButtonTextAlign, &bb);
    padding.y *= vHeightRatio;
    RenderArrow(window->DrawList, bb.Min + padding, GetColorU32(ImGuiCol_Text),
                (is_open ? ImGuiDir::ImGuiDir_Down : ImGuiDir::ImGuiDir_Right), 1.0f);
    PopID();
    return is_open;
}

bool CollapsingHeader_CheckBox(const char* vName, float vWidth, bool vDefaulExpanded, bool vShowCheckBox,
                               bool* vCheckCatched) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    const float text_base_offset_y =
        ImMax(0.0f, window->DC.CurrLineTextBaseOffset - padding.y);  // Latch before ItemSize changes it

    const ImVec2 pos = window->DC.CursorPos;
    const ImVec2 nsize =
        CalcItemSize(ImVec2(vWidth, label_size.y + style.FramePadding.y * 2.0f),
                     label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);

    ImRect bb(pos, pos + nsize);
    ImRect bbTrigger = bb;
    if (vShowCheckBox && vCheckCatched != nullptr) bbTrigger.Max.x -= nsize.y;
    ItemSize(bb, style.FramePadding.y);
    ItemAdd(bb, id);
    ...
    // Render
    const ImU32 colArrow = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive
                                       : hovered         ? ImGuiCol_HeaderHovered
                                                         : ImGuiCol_Header);
    RenderFrame(bb.Min, bb.Max, colArrow, true, style.FrameRounding);
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, vName, nullptr, &label_size,
                      style.ButtonTextAlign, &bb);
    RenderArrow(window->DrawList, bb.Min + padding + ImVec2(0.0f, text_base_offset_y), GetColorU32(ImGuiCol_Text),
                (is_open ? ImGuiDir::ImGuiDir_Down : ImGuiDir::ImGuiDir_Right), 1.0f);
    ...
    if (vShowCheckBox && vCheckCatched) {
        ...
        // render
        const ImU32 colFrame = GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive
                                           : hovered         ? ImGuiCol_FrameBgHovered
                                                             : ImGuiCol_FrameBg);
        ...
        if (*vCheckCatched) {
            ...
            RenderCheckMark(window->DrawList, bbMenu.Min + ImVec2(pad, pad - 0.1f * sizey),
                            GetColorU32(ImGuiCol_CheckMark), sizey - pad * 2.0f);
        }
    }
...
bool CollapsingHeader_Button(const char* vName, float vWidth, bool vDefaulExpanded, const char* /*vLabelButton*/,
                             bool vShowButton, bool* vButtonPressed, ImFont* /*vButtonFont*/) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ImRect bb(pos, pos + nsize);
    ImRect bbTrigger = bb;
    if (vButtonPressed && vShowButton) bbTrigger.Max.x -= nsize.y;
    ItemSize(bb, style.FramePadding.y);
    ItemAdd(bb, id);
    ...
    if (vButtonPressed && vShowButton) {
        ...
        *vButtonPressed     = ButtonBehavior(bbMenu, extraId, &menuHovered, &menuHeld, ImGuiButtonFlags_PressedOnClick);
        const ImU32 menuCol = GetColorU32((menuHovered) ? ImGuiCol_ButtonHovered
                                          : menuHovered ? ImGuiCol_HeaderHovered
                                                        : ImGuiCol_Header);
        ...
        window->DrawList->AddLine(iconPos + ImVec2(+cross_extent * 0.2f, -cross_extent * 0.2f),
                                  iconPos + ImVec2(-cross_extent, +cross_extent), GetColorU32(ImGuiCol_Text), 4.0f);
        window->DrawList->AddLine(iconPos + ImVec2(+cross_extent * 0.4f, -cross_extent * 0.4f),
                                  iconPos + ImVec2(+cross_extent, -cross_extent), GetColorU32(ImGuiCol_Text), 4.0f);
    }
...
bool SmallContrastedButton(const char* label) {
    ...
    const bool pushed = PushStyleColorWithContrast1(ImGuiCol_Button, ImGuiCol_Text, CustomStyle::puContrastedTextColor,
                                                    CustomStyle::puContrastRatio);
    PushID(++CustomStyle::pushId);
...
bool ClickableTextUrl(const char* label, const char* url, bool vOnlined) {
    ...
    bool hovered, held;
    const bool pressed = ButtonBehavior(bb, id, &hovered, &held, ImGuiButtonFlags_PressedOnClick);
    if (held || (g.HoveredId == id && g.HoveredIdPreviousFrame == id)) SetMouseCursor(ImGuiMouseCursor_Hand);
    RenderNavHighlight(bb, id);
    ...
    if (vOnlined)
        window->DrawList->AddLine(ImVec2(p0.x + style.FramePadding.x, p1.y), ImVec2(p1.x - style.FramePadding.x, p1.y),
                                  GetColorU32(col));
...
void Header(const char* vName, float width) {
    ...
    ImVec2 size = CalcItemSize(ImVec2(width, label_size.y + style.FramePadding.y * 2.0f),
                               label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, vName, NULL, &label_size,
                      style.ButtonTextAlign, &bb);
}

void ImageRect(ImTextureID user_texture_id, const ImVec2& pos, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1,
    ...
    if (border_col.w > 0.0f) {
        ...
        window->DrawList->AddImage(user_texture_id, bb.Min + pos + ImVec2(1, 1), bb.Max - ImVec2(1, 1), uv0, uv1,
...
bool ColorEdit4ForNode(const char* label, float col[4], ImGuiColorEditFlags flags) {
    ...
    // Read stored options
    if (!(flags & ImGuiColorEditFlags_DisplayMask_)) flags |= (g.ColorEditOptions & ImGuiColorEditFlags__DisplayMask);
...
bool ContrastedButton(const char* label, const char* help, ImFont* imfont, float vWidth, const ImVec2& size_arg,
    ...
    const bool res =
        ButtonEx(label, ImVec2(ImMax(vWidth, size_arg.x), size_arg.y), flags | ImGuiButtonFlags_PressedOnClick);
...
bool ToggleContrastedButton(const char* vLabelTrue, const char* vLabelFalse, bool* vValue, const char* vHelp,
                            ImFont* vImfont) {
    bool res = false;

    assert(vValue);

    const auto pushed = PushStyleColorWithContrast1(ImGuiCol_Button, ImGuiCol_Text, CustomStyle::puContrastedTextColor,
                                                    CustomStyle::puContrastRatio);
...
bool Selectable_FramedText(const char* fmt, ...) {
    ...
    const ImRect check_bb(window->DC.CursorPos,
                          window->DC.CursorPos + ImVec2(label_size.x + style.FramePadding.x * 4 - 1,
...
void ImageZoomPoint(ImTextureID vUserTextureId, const float vWidth, const ImVec2& vCenter, const ImVec2& vPoint,
...
bool InputText_Validation(const char* label, char* buf, size_t buf_size, const bool* vValidation,
                          const char* vValidationHelp, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback,
                          void* user_data) {
    if (vValidation) {
        if (*vValidation)
    ...
    if (vValidation) {
        PopStyleColor();
        if (!*vValidation && IsItemHovered()) SetTooltip("%s", vValidationHelp);
    }
    return res;
}
...
void RenderGradFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_start_col, ImU32 fill_end_col, bool border,
                     float rounding) {
    ImGuiContext& g     = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool GradButton(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags) {
    ...
    // ImGuiCol_Button); RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);

    const ImVec4 col  = GetStyleColorVec4((held && hovered) ? ImGuiCol_ButtonActive
                                          : hovered         ? ImGuiCol_ButtonHovered
                                                            : ImGuiCol_Button);
...
void ImageRatio(ImTextureID vTexId, float vRatioX, float vWidth, ImVec4 vColor, float /*vBorderThick*/) {
    ...
    ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    if (vColor.w > 0.0f) bb.Max += ImVec2(2, 2);
    ItemSize(bb);
    if (!ItemAdd(bb, 0)) return;

    if (vColor.w > 0.0f) {
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(vColor), 0.0f);
        window->DrawList->AddImage(vTexId, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), uv0, uv1,
...
#ifdef USE_OPENGL
bool TextureOverLay(float vWidth, ct::texture* vTex, ImVec4 vBorderColor, const char* vOverlayText,
                    ImVec4 vOverLayTextColor, ImVec4 vOverLayBgColor, ImVec2 vUV0, ImVec2 vUV1) {
    if (vTex == nullptr) return false;
    ...
    ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    if (vBorderColor.w > 0.0f) bb.Max += ImVec2(2, 2);
    ItemSize(bb);
    if (!ItemAdd(bb, id)) return false;
    ...
    if (vBorderColor.w > 0.0f) {
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(vBorderColor), 0.0f, ImDrawFlags_RoundCornersAll,
                                  vBorderColor.w);
        window->DrawList->AddImage((ImTextureID)(size_t)vTex->glTex, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), vUV0,
                                   vUV1, GetColorU32(ImVec4(1, 1, 1, 1)));
    } else {
        window->DrawList->AddImage((ImTextureID)(size_t)vTex->glTex, bb.Min, bb.Max, vUV0, vUV1,
                                   GetColorU32(ImVec4(1, 1, 1, 1)));
    }
...
inline float inScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, bool is_logarithmic,
...
template <typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
inline TYPE inScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic,
                                   float logarithmic_zero_epsilon, float zero_deadzone_halfsize) {
...
/*extern template IMGUI_API float ScaleRatioFromValueT<ImS32, ImS32, float>(ImGuiDataType, ImS32, ImS32, ImS32, bool,
float, float); extern template IMGUI_API float ScaleRatioFromValueT<ImS32, ImU32, float>(ImGuiDataType, ImS32, ImS32,
ImS32, bool, float, float); extern template IMGUI_API float ScaleRatioFromValueT<ImU32, ImS32, float>(ImGuiDataType,
ImU32, ImU32, ImU32, bool, float, float); extern template IMGUI_API float ScaleRatioFromValueT<ImS64, ImS64,
double>(ImGuiDataType, ImS64, ImS64, ImS64, bool, float, float); extern template IMGUI_API float
ScaleRatioFromValueT<ImU64, ImS64, double>(ImGuiDataType, ImU64, ImU64, ImU64, bool, float, float); extern template
IMGUI_API float ScaleRatioFromValueT<float, float, float>(ImGuiDataType, float, float, float, bool, float, float);
extern template IMGUI_API float ScaleRatioFromValueT<double, double, double>(ImGuiDataType, double, double, double,
bool, float, float);

extern template IMGUI_API ImS32 ScaleValueFromRatioT<ImS32, ImS32, float>(ImGuiDataType, float, ImS32, ImS32, bool,
float, float); extern template IMGUI_API ImU32 ScaleValueFromRatioT<ImU32, ImS32, float>(ImGuiDataType, float, ImU32,
ImU32, bool, float, float); extern template IMGUI_API ImS64 ScaleValueFromRatioT<ImS64, ImS64, double>(ImGuiDataType,
float, ImS64, ImS64, bool, float, float); extern template IMGUI_API ImU64 ScaleValueFromRatioT<ImU64, ImS64,
double>(ImGuiDataType, float, ImU64, ImU64, bool, float, float); extern template IMGUI_API float
ScaleValueFromRatioT<float, float, float>(ImGuiDataType, float, float, float, bool, float, float); extern template
IMGUI_API double ScaleValueFromRatioT<double, double, double>(ImGuiDataType, float, double, double, bool, float, float);
...
template <typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
inline bool inSliderBehaviorStepperT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min,
                                     const TYPE v_max, const TYPE v_step, const char* format, ImGuiSliderFlags flags,
                                     ImRect* out_grab_bb) {
    ImGuiContext& g         = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if (g.ActiveId == id) {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            ...
            if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                ...
                clicked_t = inScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
                    data_type, *v, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
...
// for sliders.
inline bool inSliderBehaviorStepper(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min,
                                    const void* p_max, const void* p_step, const char* format, ImGuiSliderFlags flags,
                                    ImRect* out_grab_bb) {
    ...
    switch (data_type) {
        case ImGuiDataType_S8: {
            ...
            bool r = inSliderBehaviorStepperT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)p_min,
                                                                   *(const ImS8*)p_max, *(const ImS8*)p_step, format,
                                                                   flags, out_grab_bb);
        ...
        case ImGuiDataType_U8: {
            ...
            bool r = inSliderBehaviorStepperT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min,
                                                                   *(const ImU8*)p_max, *(const ImU8*)p_step, format,
                                                                   flags, out_grab_bb);
        ...
        case ImGuiDataType_S16: {
            ...
            bool r    = inSliderBehaviorStepperT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32,
                                                                   *(const ImS16*)p_min, *(const ImS16*)p_max,
                                                                   *(const ImS16*)p_step, format, flags, out_grab_bb);
            if (r) *(ImS16*)p_v = (ImS16)v32;
            return r;
        }
        case ImGuiDataType_U16: {
            ImU32 v32 = (ImU32) * (ImU16*)p_v;
            bool r    = inSliderBehaviorStepperT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32,
                                                                   *(const ImU16*)p_min, *(const ImU16*)p_max,
                                                                   *(const ImU16*)p_step, format, flags, out_grab_bb);
            if (r) *(ImU16*)p_v = (ImU16)v32;
            return r;
        }
        case ImGuiDataType_S32:
            IM_ASSERT(*(const ImS32*)p_min >= IM_S32_MIN / 2 && *(const ImS32*)p_max <= IM_S32_MAX / 2);
            return inSliderBehaviorStepperT<ImS32, ImS32, float>(bb, id, data_type, (ImS32*)p_v, *(const ImS32*)p_min,
                                                                 *(const ImS32*)p_max, *(const ImS32*)p_step, format,
                                                                 flags, out_grab_bb);
        case ImGuiDataType_U32:
            IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
            return inSliderBehaviorStepperT<ImU32, ImS32, float>(bb, id, data_type, (ImU32*)p_v, *(const ImU32*)p_min,
                                                                 *(const ImU32*)p_max, *(const ImU32*)p_step, format,
                                                                 flags, out_grab_bb);
        case ImGuiDataType_S64:
            IM_ASSERT(*(const ImS64*)p_min >= IM_S64_MIN / 2 && *(const ImS64*)p_max <= IM_S64_MAX / 2);
            return inSliderBehaviorStepperT<ImS64, ImS64, double>(bb, id, data_type, (ImS64*)p_v, *(const ImS64*)p_min,
                                                                  *(const ImS64*)p_max, *(const ImS64*)p_step, format,
                                                                  flags, out_grab_bb);
        case ImGuiDataType_U64:
            IM_ASSERT(*(const ImU64*)p_max <= IM_U64_MAX / 2);
            return inSliderBehaviorStepperT<ImU64, ImS64, double>(bb, id, data_type, (ImU64*)p_v, *(const ImU64*)p_min,
                                                                  *(const ImU64*)p_max, *(const ImU64*)p_step, format,
                                                                  flags, out_grab_bb);
        case ImGuiDataType_Float:
            IM_ASSERT(*(const float*)p_min >= -FLT_MAX / 2.0f && *(const float*)p_max <= FLT_MAX / 2.0f);
            return inSliderBehaviorStepperT<float, float, float>(bb, id, data_type, (float*)p_v, *(const float*)p_min,
                                                                 *(const float*)p_max, *(const float*)p_step, format,
                                                                 flags, out_grab_bb);
        case ImGuiDataType_Double:
            IM_ASSERT(*(const double*)p_min >= -DBL_MAX / 2.0f && *(const double*)p_max <= DBL_MAX / 2.0f);
            return inSliderBehaviorStepperT<double, double, double>(bb, id, data_type, (double*)p_v,
                                                                    *(const double*)p_min, *(const double*)p_max,
                                                                    *(const double*)p_step, format, flags, out_grab_bb);
...
{
    if (fmt[0] == '%' && fmt[1] == '.' && fmt[2] == '0' && fmt[3] == 'f' && fmt[4] == 0) // Fast legacy path for "%.0f"
which is expected to be the most common case. return "%d"; const char* fmt_start = ImParseFormatFindStart(fmt);    //
Find % (if any, and ignore %%) const char* fmt_end = ImParseFormatFindEnd(fmt_start);  // Find end of format specifier,
which itself is an exercise of confidence/recklessness (because snprintf is dependent on libc or user). if (fmt_end >
fmt_start && fmt_end[-1] == 'f')
    {
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
        if (fmt_start == fmt && fmt_end[0] == 0)
            return "%d";
        ImGuiContext& g = *GImGui;
        ImFormatString(g.TempBuffer, g.TempBuffer.size(), "%.*s%%d%s", (int)(fmt_start - fmt), fmt, fmt_end); // Honor
leading and trailing decorations, but lose alignment/precision. return g.TempBuffer; #else IM_ASSERT(0 && "DragInt():
Invalid format string!"); // Old versions used a default parameter of "%.0f", please replace with e.g. "%d" #endif
    }
    return fmt;
}*/
...
// how to use this function directly. text on the left in the box for keep space value on the right in the box
bool SliderScalarCompact(float width, const char* label, ImGuiDataType data_type, void* p_data, const void* p_min,
                         const void* p_max, const void* p_step, const char* format) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ////////////////////// CUSTOM CODE ////////////
    const bool value_changed = inSliderBehaviorStepper(frame_bb, id, data_type, p_data, p_min, p_max, p_step, format,
    ...
    if (label_size.x > 0.0f && !temp_input_is_active)
        RenderTextClipped(frame_bb.Min + ImVec2(style.ItemInnerSpacing.x, 0), frame_bb.Max, label, nullptr, nullptr,
                          ImVec2(0.0f, 0.5f));
    ///////////////////////////////////////////////
...
[[maybe_unused]] bool SliderInt32Compact(float width, const char* label, int32_t* v, int32_t v_min, int32_t v_max,
                                         int32_t v_step, const char* format) {
    return SliderScalarCompact(width, label, ImGuiDataType_S32, v, &v_min, &v_max, &v_step, format);
}

bool SliderInt64Compact(float width, const char* label, int64_t* v, int64_t v_min, int64_t v_max, int64_t v_step,
                        const char* format) {
    return SliderScalarCompact(width, label, ImGuiDataType_S64, v, &v_min, &v_max, &v_step, format);
}

bool SliderUIntCompact(float width, const char* label, uint32_t* v, uint32_t v_min, uint32_t v_max, uint32_t v_step,
                       const char* format) {
    return SliderScalarCompact(width, label, ImGuiDataType_U32, v, &v_min, &v_max, &v_step, format);
}

bool SliderUInt64Compact(float width, const char* label, uint64_t* v, uint64_t v_min, uint64_t v_max, uint64_t v_step,
                         const char* format) {
    return SliderScalarCompact(width, label, ImGuiDataType_U64, v, &v_min, &v_max, &v_step, format);
}

bool SliderSizeTCompact(float width, const char* label, size_t* v, size_t v_min, size_t v_max, size_t v_step,
                        const char* format) {
...
bool SliderFloatCompact(float width, const char* label, float* v, float v_min, float v_max, float v_step,
                        const char* format) {
    return SliderScalarCompact(width, label, ImGuiDataType_Float, v, &v_min, &v_max, &v_step, format);
}

bool SliderDoubleCompact(float width, const char* label, double* v, double v_min, double v_max, double v_step,
                         const char* format) {
    return SliderScalarCompact(width, label, ImGuiDataType_Double, v, &v_min, &v_max, &v_step, format);
}

bool SliderScalarDefaultCompact(float width, const char* label, ImGuiDataType data_type, void* p_data,
                                const void* p_min, const void* p_max, const void* p_default, const void* p_step,
                                const char* format) {
    bool change = false;
...
bool SliderIntDefaultCompact(float width, const char* label, int* v, int v_min, int v_max, int v_default, int v_step,
                             const char* format) {
    return SliderScalarDefaultCompact(width, label, ImGuiDataType_S32, v, &v_min, &v_max, &v_default, &v_step, format);
}

bool SliderUIntDefaultCompact(float width, const char* label, uint32_t* v, uint32_t v_min, uint32_t v_max,
                              uint32_t v_default, uint32_t v_step, const char* format) {
    return SliderScalarDefaultCompact(width, label, ImGuiDataType_U32, v, &v_min, &v_max, &v_default, &v_step, format);
}

bool SliderSizeTDefaultCompact(float width, const char* label, size_t* v, size_t v_min, size_t v_max, size_t v_default,
                               size_t v_step, const char* format) {
    if constexpr (sizeof(size_t) == 8) {
        return SliderScalarDefaultCompact(width, label, ImGuiDataType_U64, v, &v_min, &v_max, &v_default, &v_step,
                                          format);
    }
    return SliderScalarDefaultCompact(width, label, ImGuiDataType_U32, v, &v_min, &v_max, &v_default, &v_step, format);
}

bool SliderI64DefaultCompact(float width, const char* label, int64_t* v, int64_t v_min, int64_t v_max,
                             int64_t v_default, int64_t v_step, const char* format) {
    return SliderScalarDefaultCompact(width, label, ImGuiDataType_S64, v, &v_min, &v_max, &v_default, &v_step, format);
}

bool SliderFloatDefaultCompact(float width, const char* label, float* v, float v_min, float v_max, float v_default,
                               float v_step, const char* format) {
    return SliderScalarDefaultCompact(width, label, ImGuiDataType_Float, v, &v_min, &v_max, &v_default, &v_step,
                                      format);
}

bool SliderDoubleDefaultCompact(float width, const char* label, double* v, double v_min, double v_max, double v_default,
                                double v_step, const char* format) {
    return SliderScalarDefaultCompact(width, label, ImGuiDataType_Double, v, &v_min, &v_max, &v_default, &v_step,
                                      format);
}

bool SliderScalar(float width, const char* label, ImGuiDataType data_type, void* p_data, const void* p_min,
                  const void* p_max, const void* p_step, const char* format, ImGuiSliderFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    const bool value_changed =
        inSliderBehaviorStepper(frame_bb, id, data_type, p_data, p_min, p_max, p_step, format, flags, &grab_bb);
...
bool SliderInt(float width, const char* label, int* v, int v_min, int v_max, int v_step, const char* format,
               ImGuiSliderFlags flags) {
    return SliderScalar(width, label, ImGuiDataType_S32, v, &v_min, &v_max, &v_step, format, flags);
}

bool SliderUInt(float width, const char* label, uint32_t* v, uint32_t v_min, uint32_t v_max, uint32_t v_step,
                const char* format, ImGuiSliderFlags flags) {
    return SliderScalar(width, label, ImGuiDataType_U32, v, &v_min, &v_max, &v_step, format, flags);
}

bool SliderSizeT(float width, const char* label, size_t* v, size_t v_min, size_t v_max, size_t v_step,
                 const char* format, ImGuiSliderFlags flags) {
    return SliderScalar(width, label, ImGuiDataType_U32, v, &v_min, &v_max, &v_step, format, flags);
}

bool SliderFloat(float width, const char* label, float* v, float v_min, float v_max, float v_step, const char* format,
                 ImGuiSliderFlags flags) {
    return SliderScalar(width, label, ImGuiDataType_Float, v, &v_min, &v_max, &v_step, format, flags);
}

bool SliderDouble(float width, const char* label, double* v, double v_min, double v_max, double v_step,
                  const char* format,
                 ImGuiSliderFlags flags) {
    return SliderScalar(width, label, ImGuiDataType_Double, v, &v_min, &v_max, &v_step, format, flags);
}

bool SliderScalarDefault(float width, const char* label, ImGuiDataType data_type, void* p_data, const void* p_min,
                         const void* p_max, const void* p_default, const void* p_step, const char* format,
                         ImGuiSliderFlags flags) {
    bool change = false;
...
bool SliderIntDefault(float width, const char* label, int* v, int v_min, int v_max, int v_default, int v_step,
                      const char* format, ImGuiSliderFlags flags) {
    return SliderScalarDefault(width, label, ImGuiDataType_S32, v, &v_min, &v_max, &v_default, &v_step, format, flags);
}

bool SliderUIntDefault(float width, const char* label, uint32_t* v, uint32_t v_min, uint32_t v_max, uint32_t v_default,
                       uint32_t v_step, const char* format, ImGuiSliderFlags flags) {
    return SliderScalarDefault(width, label, ImGuiDataType_U32, v, &v_min, &v_max, &v_default, &v_step, format, flags);
}

bool SliderSizeTDefault(float width, const char* label, size_t* v, size_t v_min, size_t v_max, size_t v_default,
                        size_t v_step, const char* format, ImGuiSliderFlags flags) {
    return SliderScalarDefault(width, label, ImGuiDataType_U32, v, &v_min, &v_max, &v_default, &v_step, format, flags);
}

bool SliderFloatDefault(float width, const char* label, float* v, float v_min, float v_max, float v_default,
                        float v_step, const char* format, ImGuiSliderFlags flags) {
    return SliderScalarDefault(width, label, ImGuiDataType_Float, v, &v_min, &v_max, &v_default, &v_step, format,
                               flags);
}

bool SliderDoubleDefault(float width, const char* label, double* v, double v_min, double v_max, double v_default,
                         double v_step, const char* format, ImGuiSliderFlags flags) {
    return SliderScalarDefault(width, label, ImGuiDataType_Double, v, &v_min, &v_max, &v_default, &v_step, format,
                               flags);
}
...
void DisplayAlignedWidget(const float& vWidth, const std::string& vLabel, const float& vOffsetFromStart,
...
bool BeginContrastedCombo(const char* label, const char* preview_value, ImGuiComboFlags flags) {
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton)) {
        const bool pushed = PushStyleColorWithContrast1(
            ImGuiCol_Button, ImGuiCol_Text, CustomStyle::puContrastedTextColor, CustomStyle::puContrastRatio);
...
bool BeginContrastedEditCombo(const char* label, char* preview_value, size_t preview_value_size,
...
bool ContrastedCombo(float vWidth, const char* label, int* current_item, bool (*items_getter)(void*, int, const char**),
    ...
    if (popup_max_height_in_items != -1 && !(g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint))
        SetNextWindowSizeConstraints(ImVec2(0, 0),
                                     ImVec2(FLT_MAX, inCalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
...
bool ContrastedCombo(float vWidth, const char* label, int* current_item, const char* const items[], int items_count,
                     int height_in_items) {
    PushID(++CustomStyle::pushId);

    const bool value_changed =
        ContrastedCombo(vWidth, label, current_item, inItems_ArrayGetter, (void*)items, items_count, height_in_items);
...
// "item1\0item2\0"
bool ContrastedCombo(float vWidth, const char* label, int* current_item, const char* items_separated_by_zeros,
                     int height_in_items) {
    PushID(++CustomStyle::pushId);
    ...
    bool value_changed = ContrastedCombo(vWidth, label, current_item, inItems_SingleStringGetter,
...
bool ContrastedComboVectorDefault(float vWidth, const char* label, int* current_item,
                                  const std::vector<std::string>& items, int vDefault, int height_in_items) {
    bool change = false;
...
bool ContrastedEditCombo(float vWidth, const char* label, char* buffer, size_t buffer_size, int* current_item,
...
bool InputFloatDefault(float vWidth, const char* vName, float* vVar, float vDefault, const char* vInputPrec,
                       const char* vPopupPrec, bool vShowResetButton, float vStep, float vStepFast) {
    bool change = false;
    ...
    if (IsItemActive() || IsItemHovered()) SetTooltip(vPopupPrec, *vVar);

    return change;
}

bool InputDoubleDefault(float vWidth, const char* vName, double* vVar, double vDefault, const char* vInputPrec,
                        const char* vPopupPrec, bool vShowResetButton, double vStep, double vStepFast) {
    bool change = false;
...
bool InputFloatDefaultStepper(float vWidth, const char* vName, float* vVar, float vDefault, float vStep,
                              float vStepFast, const char* vInputPrec, const char* vPopupPrec, bool vShowResetButton) {
    bool change = false;
...
bool InputDoubleDefaultStepper(float vWidth, const float& vInputOffsetFromStart, const char* vName, double* vVar,
                               double vDefault, double vStep,
                               double vStepFast, const char* vInputPrec, const char* vPopupPrec,
    ...
    Text("%s", vName);
    SameLine(vInputOffsetFromStart, GetStyle().ItemInnerSpacing.x);
    ...
    PushID(++CustomStyle::pushId);
    PushItemWidth(w);
    change |= InputDouble("##InputDouble", vVar, 0.0, 0.0, vInputPrec, ImGuiInputTextFlags_CharsScientific);
    PopItemWidth();
    PopID();
...
bool InputIntDefault(float vWidth, const char* vName, int* vVar, int vDefault, int step, int step_fast) {
    ...
    if (IsItemActive() || IsItemHovered()) SetTooltip("%i", *vVar);

    return change;
}

bool InputUIntDefault(float vWidth, const char* vName, uint32_t* vVar, uint32_t vDefault, uint32_t step,
                      uint32_t step_fast) {
    bool change = false;
...
bool ImWidgets::InputText::DisplayInputText(const float& vWidth, const std::string& vLabel,
                                            const std::string& vDefaultText, const bool& vMultiline,
                                            const float& vInputOffsetFromStart, const bool& vNeedChange) {
    bool res = false;
    float px = ImGui::GetCursorPosX();
    if (!vLabel.empty()) {
        ImGui::Text("%s", vLabel.c_str());
    ...
    if (vMultiline) {
        if (ImGui::InputTextMultiline("##ImWidgets_InputText_DisplayInputText", m_Buffer, m_Len,
...
void ImWidgets::InputText::SetText(const std::string& vText) {
    m_Text     = vText;
    size_t len = m_Len;
    if (vText.size() < len) len = vText.size();
#ifdef _MSC_VER
...
bool ImWidgets::QuickStringCombo::displayCombo(const float& vWidth, const std::string& vLabel,
    ...
    if (!m_Array.empty()) {
        ...
        change |= ImGui::ContrastedCombo(
            ...
            (void*)&m_Array, (int32_t)m_Array.size(), -1);
        ImGui::PopID();
    }
    return change;
}
...
bool ImWidgets::QuickStringEditCombo::displayCombo(const float& vWidth, const std::string& vLabel,
                                                   const float& vInputOffsetFromStart) {
    bool change = false;
    float px    = ImGui::GetCursorPosX();
    ImGui::Text("%s", vLabel.c_str());
    ...
    if (ImGui::ContrastedEditCombo(
            w, "##combo", &m_Buffer[0], m_Len, &m_Index,
            [](void* data, int idx, const char** out_text) {
                *out_text = ((const std::vector<std::string>*)data)->at(idx).c_str();
                return true;
            },
