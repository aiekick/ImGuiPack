...
IMGUI_API bool
BeginFramedGroup(const char* vLabel, ImVec2 vSize = ImVec2(0, 0), ImVec4 vCol = ImVec4(0.0f, 0.0f, 0.0f, 0.5f), ImVec4 vHoveredCol = ImVec4(0.15f, 0.15f, 0.15f, 0.5f));
...
IMGUI_API bool
ContrastedComboVectorDefault(float vWidth, const char* label, int* current_item, const std::vector<std::string>& items, int vDefault, int height_in_items = -1);
...
bool ContrastedMenuItem(const char* label, const char* help = nullptr, bool selected = false, bool enabled = true);
