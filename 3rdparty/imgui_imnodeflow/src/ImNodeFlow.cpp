...
void BaseNode::update() {
    ...
    for (auto &p : m_dynamicOuts) {
        ...
        if ((m_pos + ImVec2(titleW, 0) + m_inf->getGrid().scroll()).x < ImGui::GetCursorPos().x + ImGui::GetWindowPos().x + maxW)
            ...
            p.second->setPos(
                ImVec2((m_pos + ImVec2(titleW - p.second->calcWidth(), 0) + m_inf->getGrid().scroll()).x, ImGui::GetCursorPos().y + ImGui::GetWindowPos().y));
        p.second->update();
        p.first -= 1;
    }
    ...
    // Deleting dead pins
    m_dynamicIns.erase(
        std::remove_if(m_dynamicIns.begin(), m_dynamicIns.end(), [](const std::pair<int, std::shared_ptr<Pin>> &p) { return p.first == 0; }), m_dynamicIns.end());
    m_dynamicOuts.erase(
        std::remove_if(m_dynamicOuts.begin(), m_dynamicOuts.end(), [](const std::pair<int, std::shared_ptr<Pin>> &p) { return p.first == 0; }), m_dynamicOuts.end());
}
...
void ImNodeFlow::update() {
    ...
    // erase destroyed nodes
    for (auto &node : m_nodes) {
        if (node.second->toDestroy()) {
            m_nodes_to_destroy.push_back(node.first);
        }
    }
    if (!m_nodes_to_destroy.empty()) {
        for (const auto &uuid : m_nodes_to_destroy) {
            m_nodes.erase(uuid);
        }
