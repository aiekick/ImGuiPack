...
void BaseNode::update() {
    ...
    if (ImGui::IsKeyPressed(ImGuiKey_Delete) && !ImGui::IsAnyItemActive() && isSelected()) {
        m_inf->needToDestroyNode(m_inf->getNodes().at(m_uid));
    }
...
void ImNodeFlow::addLink(std::shared_ptr<Link>& link) { m_links.push_back(link); }
...
void ImNodeFlow::update() {
    ...
    // destroy nodes who was marked as "to destroy"
    destroyNodesIfNeeded();
...
void ImNodeFlow::needToDestroyNode(std::weak_ptr<BaseNode> vNode) { m_node_to_destroy.push_back(vNode); }

void ImNodeFlow::destroyNodesIfNeeded() {
    if (!m_node_to_destroy.empty()) {
        for (auto node : m_node_to_destroy) {
            auto ptr = node.lock();
            if (ptr != nullptr) {
                ptr->destroy();
            }
